 const { useState, useEffect, useRef, useMemo } = React;
        ...
        const getApiUrl = (endpoint) => {
            ...
            if (isPreview) {
                return `https://generativelanguage.googleapis.com/v1beta/models/${model}:${endpoint}?key=${apiKey}`;
            }
            return `${proxyBaseUrl}/v1beta/models/${model}:${endpoint}`;
        };

        // 共通セーフティ設定（誤検知によるブロックを最小化）
        ...
        function App() {
            ...
            const safeProcessImage = async (file) => {
                return new Promise((resolve) => {
                    ...
                    img.onload = () => {
                        ...
                        // サムネイル用の切り出しサイズを 80px に極小化し、データサイズを1スロットあたり1〜2KB以下に極限削減
            ...
            // 429自動リトライ分析コール (システムプロンプトのトークンダイエット版)
            const runAnalysis = async (base64, mode) => {
                let delay = 1000;
                let response;
                let success = false;
                
                // 【トークン・ダイエット】表現のブレや重複・無駄な助詞を徹底圧縮。ルール強度は100%維持。
...
- データの平滑化：各フィールドの値（Value）はすべてプレーンなフラット文字列（String）のみで出力せよ。ネストしたオブジェクト「{}」や子配列「[]」を出力することは絶対厳禁とする。`;
                ...
                if (success) {
                    try {
                        ...
                        // 【[object Object]化の防止】ネストされたオブジェクトや配列を安全に平滑化して文字列にするヘルパー
            ...
            const generatePrompt = async () => {
                ...
                try {
                    ...
                    // 【動的衣装ルーター（Dynamic Outfit Router）】の自動判別ロジック
                    ...
                    if (outfitText.match(/(水着|ビキニ|マイクロ|ストラップ|紐|紐ショーツ|tバック|チューブトップ|ビスチェ|ブラ|下着|ナイロン|swimwear|bikini|strappy|thong)/i)) {
                        // ストラップ・水着系：形状ロックと露出ロックを最大化
                        routeSpecificInstruction = `
- 【露出領域の物理的ロック】: 腕の隙間を布地で塞がれるのを防ぐため、「leaving the shoulders completely bare and the midriff fully exposed」（完全に露出した肩と、遮るもののないお腹）という上下セパレート構造を確定させる物理指示フレーズを、衣服記述の冒頭部分に必ず配置。また、「a wide and continuous expanse of bare skin is completely visible on her flat stomach between the top and bottom pieces」を盛り込め。
...
- 綿・リブニット・麻素材の部屋着化を完全に防ぐため、「sleek high-gloss wet-look spandex-nylon material」などの高光沢の化学繊維素材記述を優先させ、普通の部屋着（lounge, loungewear, ribbed cotton）は一切禁止、およびネガティブプロンプトで完全に排除（camisole, pajamas, loungewear, loose cotton fabric を記載）せよ。`;
                        ...
                        // 着崩し・羽織りもの系：アウター位置固定ロック（Drape Position Lock）
                        routeSpecificInstruction = `
...
- 無理なストラップ食い込み記述（spaghetti shoulder straps, multi-strap halter-neck 等）は衣装と矛盾して不自然になるため自動的に抑制し、和服本来の素材、帯、染め模様（large polka dots, traditional floral motifs など）の美しさと質感描写にフォーカスさせよ。`;
                        ...
                        // カジュアル・ドレス・その他の日常服：形状を美しく再現し、無理なストラップ記述は排除
                        routeSpecificInstruction = `
...
- 衣装と矛盾する「1mm spaghetti straps」や「high-cut side-tie strings」といった食い込み記述の強制挿入は行わず、対象となる衣服のテクスチャ（silk, satin, chiffon, dense knit, structured denim 等）やドレープラインの表現に美しくフォーカスせよ。`;
                    ...
                    // 【トークン・ダイエット】ルール強度は100%維持しながら余剰表現を徹底トリミング
...
3. セーフティ置換：元の衣服が「ランジェリー」等を含む場合は、必ず安全な表現（delicate strappy set, form-fitting strappy top 等）に置き換えよ。
4. FACSコードクリーン化: AUおよびADは「AU12C」「AD19」のようにコードと強度のみを反映し、名称説明は含めない。
5. 目元：対称性、黒目比率、目頭・目尻の造形、アイラインの筆致を精密に反映。
6. 禁則：プロンプト内での「CG」というワード使用は絶対禁止。実写(Photorealistic), RAW写真品質として描写。
7. 顔のパーツ配置バランス（facePlacement）の厳格英訳再現:
...
8. 非実在性の明記: AIによる架空の創作であることを示すため、"non-existent person" 等の表現を自然に組み込め。ただし「character」「virtual」「imaginary woman」「imaginary person」は絶対に使用禁止。
9. 地域・文化的背景(region): 
...
10. 印象補正(aesthetic): 
   - 「cute」時は先頭や自然な位置に "cute"、「beautiful」時は "beautiful" を追加し、顔立ちの力を極限に高めよ。`;
            ...
            // 【一括コピー機能】ポジティブとネガティブを整形して一度にコピーする
                ...
                <div className="min-h-[100dvh] bg-[#FFF8FA] text-slate-800 font-sans pb-40 overflow-x-hidden text-[12px]">
                    <header className="bg-white/90 border-b border-pink-100 p-4 sticky top-0 z-50 backdrop-blur-md flex justify-between items-center shadow-sm">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[8px] font-black text-slate-300 ml-7 tracking-widest uppercase">Version 1.6.3 Robust Memory Sync</span>
                        </div>
                    ...
                    <main className="max-w-xl mx-auto px-4 mt-4 space-y-6">
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl space-y-1.5 shadow-sm text-[9px]">
                             <div className="flex items-start gap-2">
                                 ...
                                 <div className="space-y-1">
                                    ...
                                    <p className="text-blue-600 font-bold leading-relaxed opacity-80">※読み込ませた画像は要素の抽出のみに使用され、保存されません。</p>
                                 </div>
                             </div>
                        </div>
                        ...
                        {/* UIの多重操作を物理防御 (pointer-events-none化) */}
                                    ...
                                    <div className="bg-slate-900 rounded-[2.5rem] p-8 border border-slate-800 shadow-2xl relative overflow-hidden">
                                        ...
                                        <div className="flex justify-between items-center mb-6">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                >
                                                    {copyFeedback === 'pos' ? 'COPIED!' : 'COPY POS'}
                                                </button>
                                            </div>
                                        </div>
        ...
        const loadFromSlot = (index, memorySlots, setSelections, setPreviews, setStatusMessage) => {
            ...
            // ロード時に対象の縮小サムネイル画像をプレビュー枠に完全復元する同期ロジック
            if (slot.preview) {
                setPreviews(prev => ({
                    ...prev,
                    base: slot.preview,
