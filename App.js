    ...
    const generatePrompt = async () => {
        ...
        try {
            ...
            if (outfitText.match(/(水着|ビキニ|マイクロ|ストラップ|紐|ブラ|下着|swimwear|bikini)/i)) {
                routeSpecificInstruction = `- 【露出固定】肩や胴体を広範囲に魅せる工学的アパレル表現を冒頭に配置。
- 【極小指定】"micro", "tiny"等過激語は禁止。「minimalist triangular cut」「narrow fabric panels」等幾何学的用語に変換。
- 【面積増大防止】"top", "shorts"単体禁止。構造明記。ネガティブに"full coverage, sports bra, tank top, roomwear, boxers, camisole, large cups, full back coverage"追加。
- 【形状省略防止】形状固定化指示テキストを必ず組み込め。`;
            } else if (outfitText.match(/(浴衣|ゆかた|着物|和服|和装|kimono|yukata|着崩)/i)) {
                routeSpecificInstruction = `- 【位置固定】「はだけている」場合、衣服の位置を低くロックする指示文をポジティブに組み込め。`;
            } else {
                routeSpecificInstruction = `- 衣装カッティングを幾何学的・具体的に英訳。テクスチャやドレープラインにフォーカス。`;
            }
            ...
            if (artStyleText.match(/(54:86|cheki|polaroid|instant|analog|vintage)/i)) {
                ...
                artStyleSpecificInstruction = `- 【チェキ画質】"Photorealistic"等絶対禁止。冒頭に"Lo-fi analog instant camera film, heavy grainy texture"適用。"Classic white instant photo frame with a wide, thick white border on the ${borderSide} side"を盛り込む。`;
            } else {
                artStyleSpecificInstruction = `- 【高画質】"Photorealistic, RAW photo, 8k"等を適用し精緻に反映。"non-existent person"明記。`;
            }

            const promptSystemInstruction = `最高品質の英語プロンプトをJSONで出力せよ：{"positive": "...", "negative": "..."}
【表現最適化ガイドライン】
1. 肉体直接描写("bust volume"等)禁止 ➔ "graceful feminine silhouette"等へ。
...
8. 食い込み表現("soft overspill"等)禁止 ➔ "fitting smoothly against the skin"等密着表現へ。
9. 下着名称("bra","panties"等)禁止 ➔ "two-piece ensemble", "minimalist lace-trimmed top"等へ。
10. ルーズ化防止: "top","shorts"単体禁止 ➔ 露出構造明記。ネガティブに"roomwear, camisole"追加。
11. 丈延長防止: "tank top"禁止 ➔ "underwire bralette"等へ。ネガティブに"bustier, corset"追加。
